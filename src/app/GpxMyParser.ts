...
export class GpxMyParser {
  ...
  */
  parse(gpxstring: string) {
    ...
    if (metadata != null) {
      (this.metadata as MetaData).name = this.getElementValue(metadata, "name");
      (this.metadata as MetaData).desc = this.getElementValue(metadata, "desc");
      (this.metadata as MetaData).time = this.getElementValue(metadata, "time");
  ...
  */
  calculSlope(points: Point[], cumul) {
    let slopes = [];
